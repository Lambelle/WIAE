...
generator_optimizer=tf.keras.optimizers.Adam(learning_rate=lrG,beta_1=0.9,beta_2=0.999)
discriminator_optimizer=tf.keras.optimizers.Adam(learning_rate=lrD,beta_1=0.9,beta_2=0.999)
...
class WGAN(tf.keras.Model):
    def __init__(self,discriminator,generator,decoder,de_discriminator,discriminator_extra_steps=3,gp_weight=10.0, gp_d=10.0
    ):
    ...
    def train_step(self, data):
        ...
        for i in range(self.d_steps):
            ...
            z = data
            z_ded=z[:,-(nI-2*(k_size-1)):]
            real_samples = tf.random.uniform((batch_size, nI-k_size+1),minval=-1.0,maxval=1.0)
            with tf.GradientTape(persistent=True) as tape:
                ...
                ded_cost=self.d_loss_fn(real_sample=real_ded_logits,fake_sample=fake_ded_logits)
                # Calculate the gradient penalty
                gp = self.gradient_penalty(batch_size, real_samples, fake_samples, IsDeD=False)
                # Calculate the gradient penalty for autoencoder
                gp_ded = self.gradient_penalty(batch_size, z_ded, decoded_samples_ded, IsDeD=True)
...
class GANMonitor(tf.keras.callbacks.Callback):
  ...
  def on_epoch_end(self,epoch,logs=None):
    ...
    # for i in range(output.shape[0]-b-1):
    #   np.append(output_c,output[i:i+b,:].transpose())
    # df=pd.DataFrame(output_c)
    # corr=df.corr()
    # print(df.corr)
...
def get_training_samples(dataSet,nI):
    ...
    for i in range(dataSet.size//(nI-k_size+1)-1):
        ...
        print(i)
    return x
x_train=get_training_samples(train_samples,nI)
print(x_train.shape)

cbk=GANMonitor()

wgan=WGAN(discriminator=D,generator=G,decoder=De,de_discriminator=DeD,discriminator_extra_steps=trainingC,gp_weight=gp_w,gp_d=gp_w_ded)
...
wgan.fit(x_train,batch_size=batchsize, epochs=epochs,callbacks=[cbk])
